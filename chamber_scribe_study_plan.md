...
- [ ] `docker compose up --build` (or `windows\run.bat` / `macos-linux/run.sh` + `uvicorn` locally) and watch a real video go from scraped → downloaded → transcribed in the logs.
